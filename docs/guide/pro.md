...
- **独立密码**：素匣使用独立的访问密码，与设备锁屏密码分离，即使设备被他人访问也无法直接查看素匣内容
...
素流助理内置 13 个端侧工具，覆盖文件管理和系统操作两大类核心能力：
...
| 工具 | 能力 |
|------|------|
| 日历事件 | 在系统日历中创建日程，支持设置提醒、地点和描述 |
| 闹钟设置 | 设置系统闹钟，支持重复日期 |
| 定时器 | 设置倒计时定时器 |
| 应用启动 | 按包名或应用名称启动设备上已安装的应用 |
| 打开网页 | 在浏览器中打开指定网址 |
| 系统设置 | 快速跳转到 WiFi、蓝牙、存储、电池等系统设置页面 |
...
虽然素流没有云端服务，但也通过此种方式打消用户对素流隐私、数据安全的顾虑，同时用户还可以配置更加智能的模型提升Agent能力。

### 工作方式

1. **你提供 API Key**：在应用设置中配置你自己的 LLM API Key（如 OpenAI、Anthropic 等），支持OpenAI compatible格式
...
Pro 版通过 App 内购买流程激活，也可以在官网预先购买激活码后填入 APP 激活码输入框。

在应用内点击任意 Pro 版功能，即可进入激活流程，按照引导完成购买即可解锁全部 AI 功能。购买后 Pro 版功能即时生效，无需重启应用。
